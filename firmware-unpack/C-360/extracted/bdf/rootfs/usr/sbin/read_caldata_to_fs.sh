    ...
    case "$board" in
            ...
            ap-hk01-*)
                    ...
                    if [ -f "$HK_BD_FILENAME" ]; then
                        FILESIZE=$(wc -c "$HK_BD_FILENAME" | awk '{print $1}')
                    else
                        FILESIZE=131072
                    fi
                    dd if=${mtdblock} of=${apdk}/IPQ8074/caldata.bin bs=1 count=$FILESIZE skip=4096
	    ...
	    ;;
            ap-hk* | ap-ac* | ap-oa*)
		checksum=$(md5sum /lib/firmware/IPQ8074/bdwlan.b290 | grep "cc7971989865be71a7553096cf41f9d6")
		    FILESIZE=131072
		    mkdir -p ${apdk}/IPQ8074	
		    if [ "$checksum" != "" ]
		    then
			dd if=${mtdblock} of=${apdk}/IPQ8074/caldata.bin bs=1 count=$FILESIZE skip=4096
			devmem 0x1009000 32 0x2c1
			devmem 0x1009004 32 0x0
		    else
                    	dd if=${mtdblock} of=${apdk}/IPQ8074/caldata.bin bs=1 count=$FILESIZE skip=464896
			devmem 0x1009000 32 0x2c3
                        devmem 0x1009004 32 0x2
		    fi

		ln -sf ${apdk}/IPQ8074/caldata.bin /lib/firmware/IPQ8074/caldata.bin

		mkdir -p ${apdk}/qcn9000
		dd if=${mtdblock} of=${apdk}/qcn9000/caldata_1.bin bs=1 count=$FILESIZE skip=157696
		dd if=${mtdblock} of=${apdk}/qcn9000/caldata_2.bin bs=1 count=$FILESIZE skip=311296
		ln -sf ${apdk}/qcn9000/caldata_1.bin /lib/firmware/qcn9000/caldata_1.bin
		ln -sf ${apdk}/qcn9000/caldata_2.bin /lib/firmware/qcn9000/caldata_2.bin
            ;;
            ap-cp01-c3*)
                    CP_BD_FILENAME=/lib/firmware/IPQ6018/bdwlan.bin
                    mkdir -p ${apdk}/IPQ6018
                    if [ -f "$CP_BD_FILENAME" ]; then
                        FILESIZE=$(wc -c "$CP_BD_FILENAME" | awk '{print $1}')
                    else
                        FILESIZE=65536
                    fi
            ...
            ap-mp03.1-* | db-mp03.1-* | ap-mp03.6*)
                    ...
                    if [ -f "$MP_BD_FILENAME" ]; then
						FILESIZE=$(wc -c "$MP_BD_FILENAME" | awk '{print $1}')
                    else
                        FILESIZE=131072
                    fi
                    dd if=${mtdblock} of=${apdk}/IPQ5018/caldata.bin bs=1 count=$FILESIZE skip=4096
